...
2026-07-01 更新：五张牌结果页会对卡面采用更紧凑的展示，优先保证整组牌、牌位标题和右侧初读纸面同时可读。小阿尔卡那下一步应先完善统一符号系统和 fallback 卡面密度，再决定是否生成完整 56 张插画。
...
4. 下一步：小阿尔卡那符号系统，先统一设计语言和 fallback 密度，再决定是否全量插画。
5. 后续：个人网站迁移，把工具作为 `myshkin451.com` 的一个可用页面，而不是营销落地页。

当前状态：公网 AI 解读可用，大阿尔卡那资产完成；下一轮重点应放在五张牌/移动端结果阅读体验、小阿尔卡那 fallback 质量，以及 Worker 的预算与观测能力。
